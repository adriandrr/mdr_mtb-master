...
for abres,depth in zip(abresprofile,depthprofile):
    ab_df = pd.read_csv(depth, header = 0, sep = ',')
    dp_df = pd.read_csv(abres, header = 0, sep = ',')    
    
    ab_df["Gene"]=ab_df["Gene"].str.split('_').str[0]
    ab_df = ab_df[ab_df["Mut_status"] == "res_mut"]
    dp_df["Mut_gene"]=dp_df["Mut_gene"].str.split('_').str[0]
    lsmut = []
    lslocmut = []
    for i in range(dp_df.shape[0]):
        lsmut.append(dp_df.loc[i]["Ref_Codon"]+str(dp_df.loc[i]["Codon_num"])+dp_df.loc[i]["Alt_Codon"])
        lslocmut.append(dp_df.loc[i]["Mut_gene"]+"_"+dp_df.loc[i]["Ref_Codon"]+str(dp_df.loc[i]["Codon_num"])+dp_df.loc[i]["Alt_Codon"])
    dp_df["Mutation"]=lsmut
    dp_df["LocMutation"]=lslocmut
    barchart = alt.Chart(ab_df).mark_bar(color = "#1f77b4").encode(
    ...
    base2 = alt.Chart(dp_df).transform_calculate(
    ...
    qual = alt.Chart(dp_df).mark_bar(color="#1f77b4").encode(
